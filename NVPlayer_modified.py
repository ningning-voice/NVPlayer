    ...
    def _extract_metadata(self, file_path):
        ...
        try:
            ...
            if hasattr(audio, "tags") and audio.tags:
                try:
                    for k in audio.tags.keys(): tags_obj[k] = audio.tags.get(k)
                except Exception:
                    try: tags_obj = dict(audio.tags)
                    except Exception: tags_obj = {}
            ...
            def pick(keys):
                for k in keys:
                    if k in tags_obj:
                        v = tags_obj[k]
                        try:
                            if isinstance(v, (list, tuple)): return str(v[0])
                            if hasattr(v, "text"): return str(v.text[0]) if v.text else str(v)
                            return str(v)
                        except Exception: return str(v)
                return None

            artist = pick(["artist", "ARTIST", "TPE1", "albumartist", "ALBUMARTIST"]) or "Unknown Artist"
            ...
            if not tracknumber.isdigit(): tracknumber = "0"

            return {
                "path": file_path, "artist": str(artist), "date": str(date).split("-")[0], "album": str(album),
                "title": str(title), "track": tracknumber, "duration": duration, "raw_duration": raw_duration,
                "lyrics": lyrics,
            }
        except Exception:
            return None
...
class MiniPlayer(QWidget):
    ...
    def mousePressEvent(self, event): self.old_pos = event.globalPos()
...
class AboutDialog(QDialog):
    def __init__(self, parent=None):
        ...
        if parent and "Medium" in parent.pretendard_fonts: info_text_browser.setFont(parent.pretendard_fonts["Medium"])
        else: info_text_browser.setFont(QFont("Segoe UI", 10))
...
class MetadataMusicPlayer(QWidget):
    ...
    def __init__(self):
        ...
        font_definitions = [
            ("Pretendard-Thin.otf", "Thin"), ("Pretendard-ExtraLight.otf", "ExtraLight"),
            ("Pretendard-Light.otf", "Light"), ("Pretendard-Regular.otf", "Regular"),
            ("Pretendard-Medium.otf", "Medium"), ("Pretendard-SemiBold.otf", "SemiBold"),
            ("Pretendard-Bold.otf", "Bold"), ("Pretendard-ExtraBold.otf", "ExtraBold"),
            ("Pretendard-Black.otf", "Black"),
        ]
        for filename, weight_name in font_definitions:
            ...
            if os.path.exists(font_file_path):
                ...
                if font_id != -1:
                    ...
                    if families: self.pretendard_fonts[weight_name] = QFont(families[0])
        ...
        try: locale.setlocale(locale.LC_COLLATE, "ko_KR.UTF-8")
        except locale.Error:
            try: locale.setlocale(locale.LC_COLLATE, "Korean_Korea.949")
            except locale.Error: pass
        try:
            pygame.mixer.init()
            self.pygame_initialized = bool(pygame.mixer.get_init())
            # store current mixer settings as (frequency, size, channels)
            try:
                self._mixer_settings = pygame.mixer.get_init()
            except Exception:
                self._mixer_settings = None
            if not self.pygame_initialized: QMessageBox.critical(self, "오류", "Pygame Mixer 초기화 실패.")
    ...
    def setup_menubar(self):
        ...
        if "Medium" in self.pretendard_fonts: about_action.setFont(self.pretendard_fonts["Medium"])
    ...
    def setup_ui(self):
        overall_vbox = QVBoxLayout(self)
        overall_vbox.setContentsMargins(0, 0, 0, 0)
        overall_vbox.setSpacing(0)

        self.setup_menubar()
        overall_vbox.addLayout(self.main_layout_for_menubar)
        ...
        if "SemiBold" in self.pretendard_fonts: self.tab_widget.setFont(self.pretendard_fonts["SemiBold"])
        self.artists_page = QWidget()
        self.all_tracks_page = QWidget()
        self.history_page = QWidget()
        self.tab_widget.addTab(self.artists_page, "아티스트")
        self.tab_widget.addTab(self.all_tracks_page, "전체 트랙")
        self.tab_widget.addTab(self.history_page, "재생 기록")
        self.setup_artists_page()
        self.setup_all_tracks_page()
        self.setup_history_page()

        folder_layout = QHBoxLayout()
        self.label = QLabel("음악 폴더를 선택하세요.", self)
        if "Medium" in self.pretendard_fonts: self.label.setFont(self.pretendard_fonts["Medium"])
        self.open_folder_button = QPushButton("음악 폴더 열기", self)
        if "SemiBold" in self.pretendard_fonts: self.open_folder_button.setFont(self.pretendard_fonts["SemiBold"])
        ...
        if "Bold" in self.pretendard_fonts: playlist_box.setFont(self.pretendard_fonts["Bold"])
        playlist_layout = QVBoxLayout()
        self.playlist_view = QTableWidget()
        self.playlist_view.setShowGrid(False)
        self.playlist_view.setAlternatingRowColors(True)
        self.playlist_view.setColumnCount(3)
        self.playlist_view.setHorizontalHeaderLabels(["제목", "아티스트", "길이"])
        if "Bold" in self.pretendard_fonts: self.playlist_view.horizontalHeader().setFont(self.pretendard_fonts["Bold"])
        self.playlist_view.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.playlist_view.setEditTriggers(QTableWidget.NoEditTriggers)
        self.playlist_view.verticalHeader().setVisible(False)
        self.playlist_view.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.playlist_view.itemDoubleClicked.connect(self.play_music_from_playlist)
        self.playlist_view.setContextMenuPolicy(Qt.CustomContextMenu)
        self.playlist_view.customContextMenuRequested.connect(self.show_context_menu_playlist)
        playlist_layout.addWidget(self.playlist_view)
        playlist_box.setLayout(playlist_layout)

        lyrics_box = QGroupBox("가사")
        if "Bold" in self.pretendard_fonts: lyrics_box.setFont(self.pretendard_fonts["Bold"])
        lyrics_layout = QVBoxLayout()
        self.lyrics_text = QTextEdit()
        self.lyrics_text.setReadOnly(True)
        if "Medium" in self.pretendard_fonts: self.lyrics_text.setFont(self.pretendard_fonts["Medium"])
        ...
        if "Bold" in self.pretendard_fonts: player_control_box.setFont(self.pretendard_fonts["Bold"])
        player_control_layout = QVBoxLayout()
        self.cover_label = QLabel()
        self.cover_label.setAlignment(Qt.AlignCenter)
        self.cover_label.setFixedSize(200, 200)
        self.cover_label.setStyleSheet("border: 1px solid gray; background-color: #F8F8FF; border-radius: 10px;")
        self.cover_label.setText("앨범 커버 없음")
        self.now_playing_label = QLabel("재생 중인 곡이 없습니다.")
        self.now_playing_label.setAlignment(Qt.AlignCenter)
        self.now_playing_label.setWordWrap(True)
        if "ExtraBold" in self.pretendard_fonts:
            self.now_playing_label.setFont(self.pretendard_fonts["ExtraBold"])
            self.now_playing_label.setStyleSheet("font-size: 15px; color: #111111;")
        self.audio_quality_label = QLabel("")
        self.audio_quality_label.setAlignment(Qt.AlignCenter)
        if "Medium" in self.pretendard_fonts:
            self.audio_quality_label.setFont(self.pretendard_fonts["Medium"])
            self.audio_quality_label.setStyleSheet("font-size: 11px; color: #555;")

        self.visualizer_view = QGraphicsView(self.visualizer_scene)
        self.visualizer_view.setFixedSize(200, 50)
        self.visualizer_view.setStyleSheet("border: none; background-color: transparent;")
        self.visualizer_view.setRenderHint(QPainter.Antialiasing)
        ...
        if "Medium" in self.pretendard_fonts: self.time_label.setFont(self.pretendard_fonts["Medium"])
        ...
        self.eq_button.setIcon(self.eq_icon)
        self.mini_mode_button.setIcon(self.mini_player_icon)

        self.repeat_button.setToolTip("반복 재생")
        self.shuffle_button.setToolTip("랜덤 재생")
        self.eq_button.setToolTip("이퀄라이저")
        self.mini_mode_button.setToolTip("미니 플레이어 모드")
        
        self.shuffle_button.setCheckable(True)
        
        icon_size = QSize(24, 24)
        for btn in [self.repeat_button, self.shuffle_button, self.prev_button, self.play_pause_button, self.next_button, self.stop_button, self.eq_button, self.mini_mode_button]:
        ...
        self.volume_icon.setToolTip("음소거/음소거 해제")
        self.volume_slider = QSlider(Qt.Horizontal)
        self.volume_slider.setRange(0, 100)
        self.volume_label = QLabel("0%")
        self.volume_label.setFixedWidth(40)
        if "Medium" in self.pretendard_fonts: self.volume_label.setFont(self.pretendard_fonts["Medium"])
        ...
        player_control_layout.addWidget(self.cover_label, alignment=Qt.AlignCenter)
        player_control_layout.addWidget(self.visualizer_view, alignment=Qt.AlignCenter)
        player_control_layout.addWidget(self.now_playing_label)
        player_control_layout.addWidget(self.audio_quality_label)
        player_control_layout.addWidget(self.progress_slider)
        player_control_layout.addWidget(self.time_label)
        ...
        self.play_all_tracks_button.clicked.connect(self.play_all_tracks)
    ...
    def setup_visualizer(self):
        ...
        for i in range(num_bars):
            bar = QGraphicsRectItem(start_x + i * (bar_width + bar_spacing), 0, bar_width, 1)
    ...
    def update_visualizer(self):
        if not self.pygame_initialized or not self.visualizer_view.isVisible(): return
    ...
    def setup_artists_page(self):
        ...
        if "SemiBold" in self.pretendard_fonts: self.back_button.setFont(self.pretendard_fonts["SemiBold"])
        self.artists_search_box = QLineEdit(self)
        self.artists_search_box.setPlaceholderText("검색 (아티스트, 앨범)")
        if "Medium" in self.pretendard_fonts: self.artists_search_box.setFont(self.pretendard_fonts["Medium"])
        self.artists_search_box.textChanged.connect(lambda text: self.search_music_in_view(text, "artists"))
        ...
        self.artists_list_view.customContextMenuRequested.connect(self.show_context_menu)
    ...
    def setup_all_tracks_page(self):
        ...
        if "SemiBold" in self.pretendard_fonts: self.play_all_tracks_button.setFont(self.pretendard_fonts["SemiBold"])
        self.tracks_search_box = QLineEdit(self)
        self.tracks_search_box.setPlaceholderText("검색 (제목, 아티스트, 앨범)")
        if "Medium" in self.pretendard_fonts: self.tracks_search_box.setFont(self.pretendard_fonts["Medium"])
        self.tracks_search_box.textChanged.connect(lambda text: self.search_music_in_view(text, "all_tracks"))
        ...
        if "Bold" in self.pretendard_fonts: self.track_table.horizontalHeader().setFont(self.pretendard_fonts["Bold"])
        ...
        self.track_table.horizontalHeader().sectionClicked.connect(self.sort_all_tracks_table)
            ...
            if os.path.exists("player_settings.json"):
                with open("player_settings.json", "r", encoding="utf-8") as f:
                    ...
                    if self.folder_path and os.path.exists(self.folder_path):
                        ...
                        self.start_scan_thread(self.folder_path)
        except Exception as e:
    ...
    def load_library_from_db(self):
        ...
        try:
            ...
            for track_info in music_data:
                artist, album, date, path = track_info['artist'], track_info['album'], track_info['date'], track_info['path']
                if artist not in artist_albums_covers: artist_albums_covers[artist] = {}
                if album not in artist_albums_covers[artist] or date > artist_albums_covers[artist][album].get("date", "0000"):
                    artist_albums_covers[artist][album] = {"date": date, "path": path}

            self.music_data = music_data
            self.artist_albums_covers = artist_albums_covers
            
            self.label.setText(f"총 {len(self.music_data)}곡 불러옴." if self.music_data else "음악 폴더를 선택하세요.")
    ...
    def open_folder(self):
        folder = default_dir = self.folder_path if self.folder_path else os.path.expanduser("~")
        ...
        if folder:
            ...
            msg = QMessageBox.question(self, "라이브러리 재구성", "새 폴더를 선택했습니다. 기존 라이브러리 정보를 지우고 새로 구성하시겠습니까?")
    ...
    def start_scan_thread(self, folder_path):
        if self.scan_thread and self.scan_thread.isRunning():
            QMessageBox.warning(self, "스캔 중", "이미 라이브러리 스캔이 진행 중입니다.")
            return
        self.open_folder_button.setEnabled(False)
        self.label.setText("음악 라이브러리 업데이트 중...")
    ...
    def on_scan_finished(self, added_count, deleted_count):
        ...
        if added_count > 0 or deleted_count > 0:
            QMessageBox.information(self, "라이브러리 업데이트", f"{added_count}곡 추가, {deleted_count}곡 삭제 완료.")
            self.load_library_from_db()  # 변경된 라이브러리 다시 로드
        else:
            self.label.setText(f"라이브러리가 최신 상태입니다. (총 {len(self.music_data)}곡)")
        self.open_folder_button.setEnabled(True)

    def update_progress_label(self, message): self.label.setText(self.truncate_text(message, 100))
    ...
    def show_album_cover(self, file_path, target_label=None):
        ...
        label.clear()
        rounded_pixmap = self.create_rounded_pixmap(file_path, label.width(), label.height(), 10)
        if rounded_pixmap:
    ...
    def truncate_text(self, text, max_len): return text[:max_len] + "..." if len(text) > max_len else text
    def _update_button_ui(self):
        ...
        if self.repeat_mode == 0:  # 끄기
            ...
            self.repeat_button.setIcon(self.repeat_icon)
            self.repeat_button.setStyleSheet("QPushButton { background-color: #A078D6; }")
    ...
    def keyPressEvent(self, event):
        if not self.pygame_initialized: return
        actions = {Qt.Key_Space: self.play_pause_music, Qt.Key_Right: self.play_next, Qt.Key_Left: self.play_prev}
        if event.key() in actions: actions[event.key()]()
        elif event.key() == Qt.Key_Up: self.volume_slider.setValue(min(self.volume_slider.value() + 5, 100))
        elif event.key() == Qt.Key_Down: self.volume_slider.setValue(max(self.volume_slider.value() - 5, 0))
    ...
    def play_music(self, start_pos=0.0):
        ...
        try:
            ...
            if self.is_muted: pygame.mixer.music.set_volume(self.current_volume)
            else: pygame.mixer.music.set_volume(self.current_volume)
    ...
    def update_now_playing(self):
        if self.selected_flac and 0 <= self.current_row < len(self.user_playlist):
            ...
            full_text = f"{track.get('artist', 'Unknown')} - {track.get('title', 'Unknown')}"
            self.now_playing_label.setText(self.truncate_text(full_text, 40))
            self.now_playing_label.setToolTip(full_text)
            self.lyrics_text.setText(track.get("lyrics", "가사 없음").strip() or "가사 없음")
            self.show_album_cover(track.get("path"))
            self.show_album_cover(track.get("path"), target_label=self.mini_player.cover_label)
            self.mini_player.title_label.setText(self.truncate_text(track.get("title", "곡 정보 없음"), 25))
            self.mini_player.title_label.setToolTip(track.get("title", ""))
            self.mini_player.artist_label.setText(self.truncate_text(track.get("artist", "아티스트 정보 없음"), 25))
    ...
    def play_prev(self):
        ...
        if self.is_shuffled: self.current_row = random.randrange(len(self.user_playlist))
        else: self.current_row = (self.current_row - 1 + len(self.user_playlist)) % len(self.user_playlist)
        self.play_music()
    ...
    def stop_music(self):
        ...
        self.audio_quality_label.setText("")
        self.playback_start_offset_ms = 0
        if not self.is_muted: self.volume_icon.setPixmap(self.volume_pixmap.scaled(24, 24, Qt.KeepAspectRatio))
    ...
    def on_tab_changed(self, index):
        ...
        else: self.switch_view({0: "artists", 1: "all_tracks"}.get(index))
    ...
    def update_playback_history(self, track_info):
        if track_info in self.playback_history: self.playback_history.remove(track_info)
    ...
    def play_music_from_table(self, item):
        ...
        if track_info not in self.user_playlist: self.user_playlist.append(track_info)
    ...
    def show_context_menu_playlist(self, position):
        ...
        if self.playlist_view.selectedItems():
            ...
            if "Medium" in self.pretendard_fonts: remove_action.setFont(self.pretendard_fonts["Medium"])
            remove_action.triggered.connect(self.remove_selected_from_playlist)
            menu.addAction(remove_action)
        clear_action = QAction("재생목록 비우기", self)
        if "Medium" in self.pretendard_fonts: clear_action.setFont(self.pretendard_fonts["Medium"])
    ...
    def show_context_menu(self, position):
        sender = self.sender()
        menu = QMenu(self)
        if not sender.selectedItems(): return
        
        add_action = QAction("재생목록에 추가", self)
        if "Medium" in self.pretendard_fonts: add_action.setFont(self.pretendard_fonts["Medium"])
        ...
        if sender == self.artists_list_view:
            ...
            if isinstance(item_data, str): # 아티스트 뷰
                ...
                tracks_to_add = sorted([t for t in self.music_data if t.get("artist") == artist_name], key=lambda x: (x.get("date", "0000"), locale.strxfrm(x.get("album", "")), int(x.get("track", "0"))))
            elif isinstance(item_data, dict): # 앨범 뷰
                album_info = item_data
                tracks_to_add = sorted([t for t in self.music_data if t.get("artist") == album_info.get("artist") and t.get("album") == album_info.get("album")], key=lambda x: int(x.get("track", "0")))
            ...
            for row in unique_tracks:
                ...
                if track_info: tracks_to_add.append(track_info)
        
        if tracks_to_add:
            add_action.triggered.connect(lambda: self.add_tracks_to_playlist(tracks_to_add))
    ...
    def remove_selected_from_playlist(self):
        selected_rows = sorted(set(index.row() for index in self.playlist_view.selectedIndexes()), reverse=True)
        for row in selected_rows: del self.user_playlist[row]
        self.show_playlist_view()
        self.save_playlist()

    def sort_all_tracks_table(self, logical_index):
        if self.current_sort_column == logical_index: self.sort_ascending = not self.sort_ascending
        else: self.current_sort_column, self.sort_ascending = logical_index, True
        
        sort_key = ["title", "artist", "duration"][logical_index]
        self.current_playlist.sort(key=lambda x: locale.strxfrm(str(x.get(sort_key, ""))), reverse=not self.sort_ascending)
        self.display_all_tracks_table()
    ...
    def switch_view(self, view_type):
        ...
        self.back_button.setEnabled(False)
        if view_type == "artists": self.show_artists_view()
        elif view_type == "all_tracks": self.show_all_tracks_view()

    def show_artists_view(self):
        self.current_view = "artists"
        self.artists_list_view.clear()
        unique_artists = sorted(list(self.artist_albums_covers.keys()), key=locale.strxfrm)
    ...
    def find_newest_album_cover_path(self, artist_name):
        if artist_name not in self.artist_albums_covers: return None
        newest_album = max(self.artist_albums_covers[artist_name].values(), key=lambda x: x.get("date", "0000"), default=None)
    ...
    def show_all_tracks_view(self, data_to_show=None):
        ...
        self.back_button.setEnabled(False)
        self.current_playlist = data_to_show if data_to_show is not None else self.music_data
    ...
    def display_all_tracks_table(self):
        ...
        for track in self.current_playlist:
            ...
            if album_artist != current_album_artist:
                ...
                if "ExtraBold" in self.pretendard_fonts:
                    album_font = self.pretendard_fonts["ExtraBold"]; album_font.setPointSize(12); album_item.setFont(album_font)
                album_item.setForeground(QColor("#6E41C8")); album_item.setBackground(QColor(245, 240, 250))
                album_item.setFlags(Qt.ItemIsEnabled)
                self.track_table.setSpan(row_pos, 0, 1, 3); self.track_table.setItem(row_pos, 0, album_item)
                row_pos += 1

            self.track_table.insertRow(row_pos)
            title = QTableWidgetItem("  " + track.get("title", "")); artist = QTableWidgetItem(track.get("artist", "")); duration = QTableWidgetItem(track.get("duration", "--:--"))
            if "Medium" in self.pretendard_fonts: title.setFont(self.pretendard_fonts["Medium"]); artist.setFont(self.pretendard_fonts["Medium"]); duration.setFont(self.pretendard_fonts["Medium"])
            title.setData(Qt.UserRole, track)
            self.track_table.setItem(row_pos, 0, title); self.track_table.setItem(row_pos, 1, artist); self.track_table.setItem(row_pos, 2, duration)
        self.track_table.setSortingEnabled(False)

    def show_history_view(self):
        self.history_list.clear()
        if not self.playback_history: return self.history_list.addItem("재생 기록이 없습니다.")
        for track in self.playback_history:
            item = QListWidgetItem(f"{track.get('artist', '')} - {track.get('title', '')}")
            if "Medium" in self.pretendard_fonts: item.setFont(self.pretendard_fonts["Medium"])
    ...
    def show_playlist_view(self):
        ...
        for i, track in enumerate(self.user_playlist):
            ...
            title = QTableWidgetItem(track.get("title", "")); artist = QTableWidgetItem(track.get("artist", "")); duration = QTableWidgetItem(track.get("duration", "--:--"))
            if "Medium" in self.pretendard_fonts: title.setFont(self.pretendard_fonts["Medium"]); artist.setFont(self.pretendard_fonts["Medium"]); duration.setFont(self.pretendard_fonts["Medium"])
            self.playlist_view.setItem(i, 0, title); self.playlist_view.setItem(i, 1, artist); self.playlist_view.setItem(i, 2, duration)

    def on_item_double_clicked(self, item):
        if self.current_view == "artists":
            artist_name = item.data(Qt.UserRole)
            if artist_name: self.show_artist_albums_of_artist(artist_name)
        elif self.current_view == "artist_albums":
            album_info = item.data(Qt.UserRole)
            self.user_playlist = sorted([t for t in self.music_data if t.get("artist") == album_info["artist"] and t.get("album") == album_info["album"]], key=lambda x: int(x.get("track", "0")))
            self.show_playlist_view()
            self.save_playlist()
            self.current_row = 0
            self.play_music()
    ...
    def play_all_tracks(self):
        self.user_playlist = sorted(self.music_data, key=lambda x: (locale.strxfrm(x.get("artist", "")), x.get("date", "0000"), locale.strxfrm(x.get("album", ""))))
        self.show_playlist_view()
        self.save_playlist()
        self.current_row = 0
        self.play_music()
    ...
    def show_artist_albums_of_artist(self, artist):
        ...
        self.artists_list_view.clear()
        albums = { (t.get("album"), t.get("date")): t for t in self.music_data if t.get("artist") == artist }
        sorted_albums = sorted(albums.values(), key=lambda x: x.get("date", "0000"), reverse=True)
        for album_info in sorted_albums:
            item = QListWidgetItem()
            item.setSizeHint(QSize(220, 270))
            widget = AlbumIconWidget(album_info.get("path"), album_info.get("album"), artist, album_info.get("date"), self)
            item.setData(Qt.UserRole, album_info)
            self.artists_list_view.addItem(item)
            self.artists_list_view.setItemWidget(item, widget)

    def go_back(self):
        if self.current_view == "artist_albums": self.switch_view("artists")
